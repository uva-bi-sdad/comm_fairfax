...
MAP1<-(ggplot() +
           ...
           scale_fill_gradient(low = "#E69F00", high = "#56B4E9", limits = range(obesity.map$PCT_OBESE_ADULTS13, na.rm = T), breaks = round(seq(min(obesity.map$PCT_OBESE_ADULTS13, na.rm = T), max(obesity.map$PCT_OBESE_ADULTS13, na.rm = T), length = 5), 1)) +
           coord_fixed(1.3)) +
