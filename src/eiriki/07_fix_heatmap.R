...
bchart <- ggplot(youth_results_mh_overall, aes(x = reorder(Pyramid, -as.numeric(Depressive_Symptoms)),
    ...
    theme(axis.text.x=element_text(size=23,angle=45,hjust=1,vjust=1), axis.text.y = element_text(size = 23),
          axis.title.x = element_text(size = 23),axis.title.y = element_text(size = 23)) +
...
plt <-ggplot(highSchool) +
    ...
    theme(axis.line=element_blank(),axis.text.x=element_blank(),
            ...
            axis.title.x=element_blank(), legend.key.size = unit(.5, 'in'), legend.text = element_text(size = 23),
    ...
    #geom_text_repel(data = centroids.df, aes(x = Longitude, y = Latitude, label = paste(id,round(Depressive_Symptoms,digits=0),sep='\n'))
     #                , color = "black", size = 4, nudge_x = -.005, nudge_y = -.0023)
...
#device = "png", width=20,height=11.23,scale=1)
...
bchart2 <- ggplot(youth_results_mh_overall, aes(x = reorder(Pyramid, -as.numeric(Food_Insecurity)),
    ...
    theme(axis.text.x=element_text(size=23,angle=45,hjust=1,vjust=1), axis.text.y = element_text(size = 23),
          axis.title.x = element_text(size = 23),axis.title.y = element_text(size = 23)) +
    scale_y_continuous(name="Percent", breaks = seq(5,33,5),limits=c(5, 33),oob = rescale_none)+
    guides(fill = FALSE)
...
bchart5 <- ggplot(youth_results_mh_overall, aes(x = reorder(Pyramid, -as.numeric(Extracurricular_Regularly)),
    ...
    theme(axis.text.x=element_text(size=23,angle=45,hjust=1,vjust=1), axis.text.y = element_text(size = 23),
          axis.title.x = element_text(size = 23),axis.title.y = element_text(size = 23)) +
    scale_y_continuous(name="Percent", breaks = seq(55,87,5),limits=c(55, 87),oob = rescale_none)
